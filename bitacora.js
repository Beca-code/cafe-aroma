        ...
        logs.forEach(log => {
            ...
            const row = `
                <tr>
                    <td>${fechaFormato}</td>
                    <td>${log.usuario}</td>
                    <td>${log.rol === 'admin' ? '🔐 Administrador' : '👤 Usuario'}</td>
                    <td><code style="background: #f0f0f0; padding: 4px 8px; border-radius: 4px; font-size: 11px;">${log.id_sesion}</code></td>
                    <td>${log.ip}</td>
                </tr>
            `;
            tbody.innerHTML += row;
...
async function loadCierresSesion() {
    try {
        const tabla = document.getElementById('cierresSesionTable');
        if (!tabla) return;
...
async function loadSummaryCounters() {
    try {
        ...
        document.getElementById('totalUsuarios').textContent = counters.usuariosRegistrados;
        document.getElementById('totalAccesosCorrectos').textContent = counters.accesosCorrectos;
        document.getElementById('totalAccesosFallidos').textContent = counters.accesosFallidos;
        document.getElementById('totalCierresSesion').textContent = counters.cierresSesion;
